/* ASN.1 object dumping code, copyright Peter Gutmann
   ...
   updated 21 May 2012 (version 20120521, if you prefer it that way).  
...
#define UPDATE_STRING	"21 May 2012"
...
static int displayUnicode( const wchar_t wCh, const int level )
	...
	/* Unix environments are even more broken than Win32, like Win32 the 
	   ...
	   suggest using setlocale() tricks, printf() with "%lc" as the format 
	   specifier, and others, but this doesn't seem to work either */
	if( level < maxNestLevel )
		{
#if 0
		setlocale( LC_ALL, "" );
